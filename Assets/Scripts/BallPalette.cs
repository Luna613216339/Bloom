...
public static class BallPalette
{
    ...
    public static readonly string[] ThemeNames =
    {
        ...
        "Shibuya Dark", "Aurora", "Midnight Garden", "Lava", "Frosting",
    };
    ...
    /// <summary>
    /// 定价（2026-08-02 按 30 局实测收入定的）。实测：认真打一局中位收入 96，
    /// 全部局平均 59。所以价格换算成"要打几局"是：50 ≈ 半局，300 ≈ 3 局。
    ///
    /// 结构是一个金字塔，不是一条阶梯：
    ///   0    朦胧夏日 —— 免费默认
    ///   50   青苔 / 涩谷夜·暗 —— **入场券，一浅一深**。
    ///        定这么低是为了让第一次购买发生在第一局之内，而且让玩家一开始
